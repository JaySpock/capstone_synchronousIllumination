...
% Last Modified by GUIDE v2.5 13-Feb-2020 10:59:45
...
%Initialize integration time matrices
% v1 = 0:249; %creates a vector from 0 to 249 in 1 increment steps
% v2 = 250:1;
% Thl = diag(v2); %puts vectors on the main diagonal for high to low
% Tlh = diag(v1);
...
global naneye1 keep_running fullimage
...
while keep_running
    ...
    switch choice
       ...
       case 'Calibrate'
            disp("Take a picture of a white background");
            axes(handles.axes1);
            handles.image=image;
            axis off;
            lh1 = addlistener(naneye1,'ImageProcessed', @(o,e)displayobjtesting(e,handles));
            naneye1.StartCapture();
            choice=set(handles.startbuttom,'string','Take White Picture');
            keep_running = false;
        %can add additional sequences for manually ending each capture
        %sequence if required
        case 'Take White Picture'
            disp("Take a picture of a black background");
            %will we need anything else here? - since things will actually
            %be controlled with a switch on the FPGA
            choice=set(handles.startbuttom,'string','Take Black Picture');
            keep_running = false;

        case 'Take Black Picture'
            disp("Calibration finished");
            naneye1.StopCapture();
            choice=set(handles.startbuttom,'string','Calibrate');
            keep_running = false;
